...
pub fn parse(
    ...
    let variables: TokenStream = data_struct
        ...
        .filter_map(|field| match field.get_type() {
            Field::FieldStruct { struct_name } => {
                build_default_value(&field, Some(quote!(#struct_name)))
            }
        ...
        .filter(|field| {
            ...
            match field.get_type() {
                ...
                Field::FieldOption { data_type } => {
                    !matches!(*data_type, Field::FieldStruct { .. })
                }
    ...
    let attributes_loading: TokenStream = data_struct
        ...
        .filter_map(|field| {
            ...
            match field.get_type() {
                ...
                Field::FieldStruct { struct_name } => {
                    visit_struct(struct_name, quote! { = Some(value) })
                }
    ...
    .map(|field| {
      ...
      let validation = field
        .get_validation()
        .map(|validation_str| validation_str.parse::<TokenStream>().unwrap());
      let validation_error = format!("invalid value in field {}", name);
      match field.get_type() {
        Field::FieldOption { .. } => {
          if validation.is_some() {
            let attr_name = field.renamed_label_without_namespace();
            quote! {
              #label: match &#value_label {
                Some(val) => {
                    if #validation::validate(&val) {
                        ...
                        Err(format!("invalid value in attribute {}", #attr_name))?
                    }
                }
                None => #value_label,
              },
        ...
        Field::FieldVec { .. } => {
          if validation.is_some() {
            let attr_name = field.renamed_label_without_namespace();
            quote! {
              #label: if #value_label.len() > 0 {
                let is_valid = &#value_label.iter().all(|val| #validation::validate(&val));
                if *is_valid {
                  #value_label
                } else {
                  Err(format!("invalid value in field {}", #attr_name))?
                }
              } else {
                #value_label
              }
    ...
    quote! {
      impl #impl_generics ::yaserde::YaDeserialize for #name #ty_generics #where_clause {
        ...
        fn deserialize<R: ::std::io::Read>(
          ...
          // println!("Struct {} @ {}: start to parse {:?}", stringify!(#name), start_depth,
          //        named_element);
          ...
          #visit_unused
          // println!("Struct {} @ {}: success", stringify!(#name), start_depth);
