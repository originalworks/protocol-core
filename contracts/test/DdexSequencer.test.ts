...
describe("DdexSequencer", () => {
      ...
      await expect(
        ddexSequencer.contract
          ...
          .submitNewBlob(imageId, kzgInput.commitment, kzgInput.blobSha2, {
            ...
            blobs: [
              {
                data: new Uint8Array(kzgInput.blobFile),
    ...
    it("Clear queue after submitting proof for the last message", async () => {
      ...
      await (
        await ddexSequencer.contract
          ...
          .submitProof(imageId, JOURNAL_EXAMPLE, "0x00000000", "ipfscid")
      ).wait();
  ...
  describe("ImageId compatibility", () => {
    it("Rejects on incompatible blob imageId", async () => {
      ...
      // rejects: bytes32(0)
      await expect(submitBlob(ZERO_BYTES32, kzgInput1)).to.be.revertedWith(
      ...
      ).to.be.revertedWith("DdexSequencer: Unsupported imageId");
    ...
    it("Rejects on incompatible verifier imageId", async () => {
      ...
      await expect(
        ddexSequencer.contract
          .connect(validators[0])
          .submitProof(ZERO_BYTES32, JOURNAL_EXAMPLE, "0x00000000", "ipfscid"),
      ).to.be.revertedWith("DdexEmitter: ImageId cannot be 0");
      ...
      ).to.be.revertedWith("DdexEmitter: Unsupported imageId");

      // success: imageId == currentBlobImageId
      await expect(
        ddexSequencer.contract
          .connect(validators[0])
          .submitProof(ddexEmitter.imageId, JOURNAL_EXAMPLE, "0x00000000", "ipfscid"),
      ).not.to.be.reverted;
  ...
  describe("Blob assignment", () => {
    ...
    it("Can't assign BLOB when all blobs were already assigned", async () => {
      ...
      await expect(ddexSequencer.contract.assignBlob()).to.reverted;
    });
  ...
  describe("Blob expiry", () => {
    it("Rejects to remove exipred blob from queue", async () => {
      ...
      ).to.be.revertedWith("DdexSequencer: Blob is still considered alive");
    });
